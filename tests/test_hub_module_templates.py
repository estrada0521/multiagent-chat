...
class HubModuleTemplateTests(unittest.TestCase):
    ...
    def test_hub_home_html_resolves_module_tokens(self) -> None:
        ...
        self.assertIn("transform: translate3d(100%, 0, 0);", mobile)
        self.assertIn("#chatOverlay.overlay-closing {", mobile)
        self.assertIn("#chatOverlay.overlay-dragging {", mobile)
        self.assertIn("const CHAT_OVERLAY_CLOSE_MS = 340;", mobile)
        self.assertIn("const CHAT_OVERLAY_SWIPE_CANCEL_MS = 220;", mobile)
        self.assertIn('e.data.type === "multiagent-chat-back-swipe"', mobile)
