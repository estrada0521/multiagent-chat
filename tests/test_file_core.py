...
class FileCoreTests(unittest.TestCase):
    ...
    def test_file_view_text_keeps_line_number_and_code_line_height_in_sync(self) -> None:
        ...
        self.assertIn("--preview-gutter-bg:transparent;", page)
        ...
        self.assertNotIn('.code-gutter::before{content:"";position:absolute;inset:0 auto 0 0;width:1px;background:var(--preview-gutter-divider)}', page)
        ...
        self.assertIn('.code-gutter-table tbody tr.is-selected .ln,.code-table tbody tr.is-selected .lc{background:var(--preview-selected-line-bg)}', page)
        self.assertIn('codeScroll?.addEventListener("wheel",verticalBiasWheel,{passive:false});', page)
        self.assertIn('const gutterSyncScroll=document.getElementById("codeScroll");', page)
        self.assertIn('gutterSyncSurface?.addEventListener("wheel",forwardGutterWheel,{passive:false});', page)
    ...
    def test_file_view_progressive_text_uses_line_number_table_and_small_chunks(self) -> None:
        ...
        self.assertIn('id="codeGutterBody"', page)
        self.assertIn('class="code-table"', page)
        self.assertIn("const chunkBytes=32768;", page)
        self.assertIn("headers:{Range:`bytes=${start}-${end}`}", page)
        self.assertIn('data-preview-title-offset="', page)
        self.assertIn('const progressiveGutterBody=document.getElementById("codeGutterBody");', page)
        self.assertIn("const gutterRows=[];", page)
        self.assertIn("progressiveGutterBody.insertAdjacentHTML('beforeend',gutterRows.join(''));", page)
    ...
    def test_file_view_html_can_toggle_between_web_and_text_modes(self) -> None:
        ...
        self.assertNotIn('.html-preview-gutter::before{content:"";position:absolute;inset:0 auto 0 0;width:1px;background:var(--preview-gutter-divider)}', page)
        ...
        self.assertIn('const gutterSyncScroll=document.getElementById("htmlTextCodeScroll");', page)
        ...
        self.assertIn('const selectableGutter=document.querySelector(".html-preview-gutter-table");', page)
