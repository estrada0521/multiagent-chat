...
class ChatAssetsTests(unittest.TestCase):
    ...
    def test_mobile_header_action_buttons_use_ios_press_feedback(self) -> None:
        ...
        self.assertIn("--chrome-icon-btn-size: 36px;", style)
        self.assertIn("--chrome-icon-size: 18px;", style)
        self.assertIn("--chrome-icon-gap: 6px;", style)
        self.assertIn("padding: max(8px, env(safe-area-inset-top)) calc(var(--page-side-pad) + env(safe-area-inset-right, 0px)) 8px var(--page-side-pad);", style)
        self.assertNotIn(".shell > .hub-page-header .hub-page-header-actions .hub-page-menu-btn {", style)
        self.assertNotIn("is-tap-bloom", style)
        self.assertNotIn("@keyframes mobileHeaderBtnTapBloom", style)
    ...
    def test_mobile_hamburger_supports_ios_native_picker_menu(self) -> None:
        self.assertNotIn('id="hubPageNativeMenuSelect"', chat_assets.CHAT_MOBILE_HTML)
        self.assertNotIn('id="hubPageNativeMenuSelect"', chat_assets.CHAT_HTML)
        mobile_style = chat_assets.CHAT_MOBILE_MAIN_STYLE_ASSET
        self.assertIn(".shell > .hub-page-header > #hubPageMenuPanel.ios-native-menu:not(.hub-menu-mode-pane) {", mobile_style)
        self.assertIn(".hub-page-menu-item .action-icon {", mobile_style)
        self.assertIn(".hub-page-menu-item .action-mobile {", mobile_style)
        self.assertIn("display: none;", mobile_style)
        mobile_script = chat_assets.CHAT_MOBILE_APP_SCRIPT_ASSET
        self.assertIn("const useNativeHeaderMenuSheet = !!(_isMobile && isAppleTouchDevice && rightMenuPanel && rightMenuBtn);", mobile_script)
        self.assertIn('rightMenuPanel.classList.add("ios-native-menu");', mobile_script)
        self.assertNotIn("nativeHeaderMenuSelect", mobile_script)
