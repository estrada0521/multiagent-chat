...
<body class="hub-view-__HUB_VIEW__">
  ...
  <script>
    ...
    const renderStats = (statsData = {}) => {
      ...
      const messageSenderRows = ["user", __SELECTABLE_AGENT_NAMES_JS__]
        .map((key) => ({ label: formatLabel(key), value: Number((statsData.messages_by_sender || {})[key] || 0) }))
        .filter((item) => item.value > 0)
        .map((item) => ({ label: item.label, value: String(item.value) }));
      const activatedAgentCount = [__SELECTABLE_AGENT_NAMES_JS__]
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .dir-actions { display: flex; border-top: 0.5px solid var(--line); background: rgba(255,255,255,0.02); }
    .dir-actions button { flex: 1; padding: 12px 14px; text-align: center; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; color: var(--fg); cursor: pointer; background: transparent; border: none; font-family: inherit; transition: background 150ms ease; }
    .dir-actions button:first-child { border-right: 0.5px solid var(--line); }
    .dir-actions button:hover { background: rgba(255, 255, 255, 0.06); }
...
<body>
      ...
      <div class="workspace-stack">
        ...
        <div class="dir-browser" id="dir-browser" hidden>
          ...
          <div class="dir-actions">
            <button type="button" id="dir-mkdir">New Folder</button>
            <button type="button" id="dir-use">Use This Folder</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    const dirMkdir = document.getElementById("dir-mkdir");
    ...
    dirMkdir.addEventListener("click", async () => {
      if (!currentDirPath) return;
      const name = prompt("Enter new folder name:");
      if (!name || !name.trim()) return;
      const path = currentDirPath + (currentDirPath.endsWith("/") ? "" : "/") + name.trim();
      try {
