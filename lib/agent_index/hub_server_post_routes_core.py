...
def post_push_subscribe(
    ...
    *,
    repo_root,
    upsert_hub_push_subscription_fn,
    hub_push_monitor,
) -> None:
    ...
    if err:
        ...
        return
    try:
        result = upsert_hub_push_subscription_fn(
            repo_root,
...
def post_push_unsubscribe(
    ...
    *,
    repo_root,
    remove_hub_push_subscription_fn,
) -> None:
    data, err = _read_json_body(handler)
    if err:
        handler._send_json(400, {"ok": False, "error": err})
    ...
    if not endpoint:
        ...
        return
    try:
        removed = remove_hub_push_subscription_fn(repo_root, endpoint)
    except Exception as exc:
        handler._send_json(500, {"ok": False, "error": str(exc)})
        return
...
def post_push_presence(handler, *, hub_push_monitor) -> None:
    data, err = _read_json_body(handler)
    if err:
        handler._send_json(400, {"ok": False, "error": err})
        return
