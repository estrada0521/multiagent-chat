...
{_bh_agent_detail_selectors()} {{
    ...
    def agent_launch_cmd(self, agent_name):
        ...
        extra = adef.launch_extra if adef else ""
        flags = adef.launch_flags if adef else ""
        cmd = f"exec {extra + ' ' if extra else ''}{agent_exec}{' ' + flags if flags else ''}"
        parts.append(cmd)
        return "; ".join(parts)
    ...
    def agent_resume_cmd(self, agent_name):
        ...
        if adef.launch_env:
            parts.append(f"export {adef.launch_env}")
        extra = adef.launch_extra if adef.launch_extra else ""
        resume = adef.resume_flag
        extra_flags = adef.resume_extra_flags
        cmd = f"exec {extra + ' ' if extra else ''}{agent_exec} {resume}{' ' + extra_flags if extra_flags else ''}"
        parts.append(cmd)
        return "; ".join(parts)
