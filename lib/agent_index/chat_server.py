...
def git_commit_file(*, rel_path: str, message: str, agent: str = ""):
    root = Path(workspace or _repo_root).resolve()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/git-branch-overview":
            ...
            try:
                body = json.dumps(
                    git_branch_overview(offset=raw_offset, limit=raw_limit),
                    ensure_ascii=True,
                ).encode("utf-8")
            except Exception as exc:
    ...
    def do_POST(self):
        ...
        if parsed.path == "/git-commit-file":
            ...
            if not message:
                ...
                return
            try:
                result = git_commit_file(rel_path=rel, message=message, agent=agent)
            except PermissionError:
                self._send_json(403, {"ok": False, "error": "outside workspace"})
                return
            except ValueError as exc:
        ...
        if parsed.path == "/git-commit-all":
            ...
            if not message:
                ...
                return
            try:
                result = git_commit_all(message=message, agent=agent)
            except ValueError as exc:
                self._send_json(400, {"ok": False, "error": str(exc)})
                return
            except Exception as exc:
        ...
        if parsed.path == "/git-restore-file":
            ...
            if not rel:
                ...
                return
            try:
                result = git_restore_file(rel_path=rel)
            except PermissionError:
                self._send_json(403, {"ok": False, "error": "outside workspace"})
                return
            except ValueError as exc:
