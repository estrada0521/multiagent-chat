...
スマホ対応は補助的な viewer ではありません。既存 session の確認だけでなく、新規 session 作成や chat UI での操作も同じ流れで行えます。独自ドメインや Cloudflare を使う構成は後段の話で、最初に触れる remote control は LAN 上の利用です。

### 1. New Session / Body

事実として、この段階で見てほしい点は次のとおりです。
...
メッセージは単なるプレーンテキストではなく、後から辿れる構造を持っています。`msg-id`、`reply-to`、ファイル参照、コピー導線が UI 上に露出しているため、長い会話でも追跡しやすい構成です。

### 1.5. Thinking / Pane Trace

ここでは更新間隔と PC / スマホ差分が重要です。
...
Pane Trace は pane の直近状態を見るための画面です。`.agent-index.jsonl` が会話ログであるのに対し、こちらは pane 側の表示を追うためのものです。
...
diff 内のファイル名は外部エディタへつながっています。会話と差分確認を同じ画面系で行えます。
...
プレビューで足りない場合は、そのまま editor 側へ移れます。
...
最初に触れるべき外部アクセスは LAN 利用です。

- 同一 Wi-Fi / LAN 上のスマホから Hub を開けます。
- スマホ側でも New Session と chat UI 操作が可能です。

その後の拡張として、独自ドメインや Cloudflare を使った公開構成があります。これは追加設定を前提にした運用です。
