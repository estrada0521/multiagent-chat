...
The per-session chat UI is served by `bin/agent-index`. `ChatRuntime.payload()` now delegates JSON document shaping to `chat_payload_core.py`, and returns payload data containing `session`, `workspace`, `port`, `targets`, and `entries`. Front-end bootstrap payload shaping is also split into `chat_bootstrap_core.py` so `chat_assets.py` focuses on rendering. KaTeX and Mermaid are rendered in that front-end layer.
