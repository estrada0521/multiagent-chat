...
```bash
printf '%s' 'docs/AGENT.md を読みました。この環境での返答経路、--reply、添付、ログ参照の前提を把握しました。' | agent-send --reply <msg-id> user
```
...
```bash
printf '%s' '確認しました。' | agent-send --reply <msg-id> user
```
...
2. 必要なら `agent-index` や `.agent-index.jsonl` で `msg-id` を探す
