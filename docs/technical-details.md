...
| `lib/agent_index/hub_core.py`       | Hub runtime の facade（session query / stats / chat supervisor helper へ委譲）                                  |
| `lib/agent_index/hub_session_query_core.py` | active / archived session の収集、index preview 組み立て、session record 構築                                  |
| `lib/agent_index/hub_stats_core.py` | agent ごとの status 判定と Hub stats 集計                                                                          |
| `lib/agent_index/hub_chat_supervisor_core.py` | chat server の readiness/match 判定、起動/停止、revive/kill/delete lifecycle 操作                               |
| `lib/agent_index/hub_settings_crons_view_core.py` | Hub settings/cron の view-model 組み立てと font choice helper |
...
| `/cron`             | 現在の session / target を埋めた quick-create Cron flow を開く |
