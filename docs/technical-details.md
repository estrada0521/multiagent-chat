...
| `bin/lib/multiagent_cmd_dispatch.sh` | command mode ハンドラ分離（`status/context/list/save/resume/kill/rename/add/remove`） |
| `bin/lib/multiagent_agent_rules_core.sh` | session 起動時の provider 別 agent rule/trust 設定ヘルパ |
...
| `bin/agent-index`                   | Hub、chat UI、Settings、upload / trace / export などの HTTP endpoint                                           |
| `lib/agent_index/chat_core.py`      | chat server runtime の facade / orchestration（provider sync・delivery・status・lifecycle helper へ委譲） |
| `lib/agent_index/chat_sync_providers_core.py` | provider 別 sync adapter（Claude/Codex/Cursor/Copilot/OpenCode + 共通 wiring） |
| `lib/agent_index/chat_sync_providers_qwen_gemini_core.py` | Qwen/Gemini の sync adapter を主モジュールから分離 |
| `lib/agent_index/chat_sync_state_core.py` | sync state 永続化、global/native log claim 調停、cursor status/handoff ヘルパ |
| `lib/agent_index/chat_status_core.py` | pane status / running state 判定と runtime event 抽出 |
| `lib/agent_index/chat_agent_lifecycle_core.py` | agent launch/resume コマンド組み立てと pane restart/resume ヘルパ |
| `lib/agent_index/chat_delivery_core.py` | `send_message` 配送制御、pane prompt/trust 判定、send cooldown ヘルパ |
| `lib/agent_index/chat_trace_core.py` | trace endpoint / popup 向け pane trace 取得・整形ヘルパ |
...
| `lib/agent_index/chat_assets.py`    | chat UI の HTML / CSS / JavaScript、composer、memory、Pane Trace                                           |
| `lib/agent_index/chat_assets_script_core.py` | chat header/action markup と chat-app script asset 抽出 |
| `lib/agent_index/chat_routes_assets.py` | chat の asset/static/page 系ルート（`/`、PWA/static、icon/font、notify sound） |
| `lib/agent_index/chat_routes_read.py` | chat の read ルート群（`/messages`、trace、file/session/git read） |
| `lib/agent_index/chat_routes_write.py` | chat の write ルート群（`/send`、add/remove agent、memory/upload/git write） |
| `lib/agent_index/chat_routes_push.py` | chat の push ルート群（`/push-config`、`/push/*`） |
| `lib/agent_index/hub_core.py`       | Hub runtime の facade（session query / chat supervisor helper へ委譲）                                  |
| `lib/agent_index/hub_session_query_core.py` | active / archived session の収集、index preview 組み立て、session record 構築                                  |
| `lib/agent_index/hub_chat_supervisor_core.py` | chat server の readiness/match 判定、起動/停止、revive/kill/delete lifecycle 操作                               |
| `lib/agent_index/hub_settings_view_core.py` | Hub settings の view-model 組み立てと font choice helper |
...
| `lib/agent_index/state_core.py`     | Hub settings、chat port の永続化                                                                      |
...
| `bin/multiagent-public-edge`        | main local workflow とは分けて扱う optional reverse-proxy helper（reference docs 側で説明）                                                           |
...
`bin/multiagent` は tmux session を作成し、workspace、log directory、tmux socket、pane ID、agent 一覧を `MULTIAGENT_*` 環境変数として session に書き込みます。同じ base agent が複数回指定された場合は、`claude-1`、`claude-2` のように instance suffix を付けて pane 変数を一意にします。`multiagent add-agent` と `multiagent remove-agent` もこのレイヤの操作です。加えて、`--user-pane` の仕様解析、topology lock / state file 更新、tmux 環境からの session context 解析、add/remove 時の agent一覧遷移ロジック、pane 起動コマンド組み立て、tmux の pane/window 変更（retile・作成/分割・削除・既定 option 設定）は `multiagent_topology_core.py`、`multiagent_state_core.py`、`multiagent_session_core.py`、`multiagent_agent_core.py`、`multiagent_launch_core.py`、`multiagent_tmux_core.py` に分離され、shell 依存を減らしつつ単体テスト可能な境界を作っています。さらに save/index mirror/system entry 系、user-pane/send 系、command mode dispatch 系、provider 別 startup rule 設定系のヘルパを `bin/lib/multiagent_save_core.sh`、`bin/lib/multiagent_user_pane_core.sh`、`bin/lib/multiagent_cmd_dispatch.sh`、`bin/lib/multiagent_agent_rules_core.sh` へ切り出し、CLI entry script の肥大化を抑えています。

chat UI は `bin/agent-index` から session ごとに配信され、`ChatRuntime.payload()` は `chat_payload_core.py` に委譲して `session`、`workspace`、`port`、`targets`、`entries` をまとめた JSON を返します。さらに provider 別 sync は `chat_sync_providers_core.py` と `chat_sync_providers_qwen_gemini_core.py` に分割し、sync state / claim 調停は `chat_sync_state_core.py`、pane status/runtime 抽出は `chat_status_core.py`、pane trace 取得は `chat_trace_core.py` に分離、delivery/lifecycle は `chat_delivery_core.py` と `chat_agent_lifecycle_core.py` へ委譲しています。これにより `chat_core.py` は orchestration/facade 寄りの責務へ整理されています。加えて front-end 初期化用の bootstrap payload は `chat_bootstrap_core.py`、Pane Trace ポップアップ状態は `chat_pane_trace_core.py`、chat template placeholder 置換用の描画前データは `chat_render_core.py` に分離し、header/script 抽出は `chat_assets_script_core.py` へ寄せ、`chat_assets.py` は描画責務に集中しています。HTTP ルート処理は `chat_routes_assets.py` / `chat_routes_read.py` / `chat_routes_write.py` / `chat_routes_push.py` の route family に分割し、`chat_server.py` は family dispatcher で振り分ける構造に整理しました。KaTeX と Mermaid の render は引き続き front-end で行います。

New Session は現在、tmux を即起動しません。Hub は `/start-session-draft` で pending draft session を先に作り、その draft 用 chat runtime を起動して `launch_pending` のまま composer を開きます。最初の送信時にだけ、選択されている初期 agent と一緒に pending が解除され、通常の tmux 起動経路へ handoff されます。
...
chat UI からの通常送信は `POST /send` を通り、`ChatRuntime.send_message()` が target pane へ直接配送しつつ `sender="user"` の JSONL entry を追記します。`/memo` は pane 配送を行わず user 宛てメモ entry を直接 JSONL に残し、target 未選択の通常送信も同じ self 宛てフローにフォールバックします。

## 1.5. Thinking / Pane Trace

`ChatRuntime.agent_statuses()` は各 pane の直近 20 行を `tmux capture-pane` で取り、前回 snapshot と比較して `running` / `idle` / `dead` / `offline` を判定します。内容が変わった直後は grace period のあいだ `running` を維持し、変化が止まると `idle` へ戻します。

Pane Trace は `GET /trace?agent=<name>&lines=<n>` で取得します。`trace_content()` は `tmux capture-pane -p -e` を使い、tail mode では最後の数十行だけ、重い path では長い scrollback を返します。front-end は現在表示中の tab だけを poll し、遅延の大きい経路では自動で間隔を落とします。
...
composer は `chat_assets.py` 内の overlay と quick action 群で構成されています。通常は閉じておき、モバイルでは `O` ボタン、デスクトップでは `O` ボタンまたは middle click で開きます。textarea の下には send button、mic button、Import、memory / save log / pane control が並びます。
...
| ------------------- | ---------------------------------------------------- |
| `/memo [text]`      | `user` 自身を target にして send。本文が空でも Import 添付があれば送信可（target 未選択の通常送信も self 宛）   |
| `/load`             | 現在の `memory.md` を selected agent に送る               |
| `/memory`           | selected agent に memory 更新要求を送る                    |
...
`@` 補完は workspace path を会話へ差し込むための front-end 機能です。file existence の確認には `/files-exist` が使われます。Import は別の経路で、`POST /upload` にファイル本体を送り、`logs/<session>/uploads/<timestamp>_<hex>.<ext>` へ保存します。保存後は workspace 相対 path が返され、chat 本文には file card として出ます。加えて、インラインコード（例: `` `lib/agent_index/chat_core.py` ``）は規則ベースで path 解決し、解決できた場合は file preview リンクに変換されます。
...
Camera mode は header menu の `Camera` から開く mobile-first overlay です。`openCameraMode()` が target agent を決め、full-screen の `getUserMedia()` surface を開き、その上に最近の agent reply を重ねて表示します。

撮影画像は生 blob のまま送るのではなく、`captureCameraModeFrameBlob()` で video frame を canvas に落とし、`resizeCameraModeBlob()` で縮小（現状 max side 1280px、JPEG quality 0.7）した後、`POST /upload` で `logs/<session>/uploads/` に保存し、最後に `POST /send` で通常の `[Attached: ...]` message として selected target に送ります。同じ overlay 内の voice input は Web Speech API と hybrid waveform を使い、別 audio stream を開ける環境では live Web Audio の波形に切り替え、競合や policy 制約で取れない環境では CSS fallback animation を維持します。
...
`ChatRuntime.ensure_commit_announcements()` は `git log -1` と前回 state を比べ、commit が進んでいれば `kind="git-commit"` の system entry を JSONL に追加します。これにより code history が chat / export 側にも見えるまま残ります。

### File Menu

file menu は `FileRuntime` が担当します。raw file 配信は `/file-raw`、text content は `/file-content`、editor handoff は `/open-file-in-editor` です。`FileRuntime` は extension と file 内容から preview mode を選び、Markdown、画像、PDF、動画、音声、plain text を切り替えます。
...
desktop 側の Hub は、session 一覧 sidebar と埋め込み chat を同じ page に持つ workbench になりました。session 切り替え時は whole-page navigation ではなく cached chat URL と prewarm を使って切り替えます。旧来の standalone error page も廃止され、失敗時は pending error を持って Hub へ戻る形です。

settings は `state_core.load_hub_settings()` と `save_hub_settings()` を通ります。Hub / chat 共通の設定は repo-local ではなく local state directory に保存されます。chat port override は `.chat-ports.json`、Hub settings は `.hub-settings.json` に保存されます。現行の settings surface はかなり絞られており、black-hole を基準 theme としつつ、font、text size、Auto mode、Awake、通知、bold mode を中心に扱います。
...
`.agent-index.jsonl` と pane logs の役割は分かれています。JSONL は message routing の canonical log で、pane logs は terminal 表示の snapshot です。Hub preview、reply preview、agent-to-agent 履歴の再構成は JSONL を基準にし、Pane Trace や `.log` / `.ans` は pane 側の text を確認したいときに使います。
...
local HTTPS は、同一マシンまたは LAN 上で通知、マイク、PWA install などの secure browser feature を有効にするためのものです。

外部公開系の tooling は main workflow とは分けて扱い、`docs/reference/public-access/` 配下の reference docs に退避しています。

## 関連文書

- [README.md](../README.md): ローカル用の機能概要
- [docs/design-philosophy.md](design-philosophy.md): なぜその構成を選んでいるか
- [docs/AGENT.md](AGENT.md): この環境で動く agent 向けガイド
- [docs/reference/README.ja.md](reference/README.ja.md): main workflow から分けた reference docs
