...
- 主なエラーコード:
  ...
  - `404`: 対象なし
  - `500`: 実行時エラー
- サーバ側ハンドラ自体には認証層がないため、基本は trusted local 環境で使ってください。
...
| Path | 用途 | 入力 | 出力 |
|---|---|---|---|
| `/sessions` | active/archived 一覧 + message stats | なし | `{ sessions, active_sessions, archived_sessions, stats, tmux_state, tmux_detail }` |
