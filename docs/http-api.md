...
- 主なエラーコード:
  ...
  - `404`: 対象なし
  - `500`: 実行時エラー
- サーバ側ハンドラ自体には認証層がないため、公開時は外側でアクセス制御してください。
...
| `/briefs` | brief 一覧 | なし | `{ briefs: [...] }` |
| `/brief-content` | brief 読み込み | `name?` | `{ name, path, content, exists }` |
...
| Path | 用途 | 入力 | 出力 |
|---|---|---|---|
| `/send` | メッセージ送信 / provider direct | JSON `{ target, message, reply_to?, silent?, raw?, provider_direct?, provider_model? }` | `{ ok, ... }` |
...
| `/brief-content` | brief 保存 | JSON `{ name, content }` | `{ ok, name, path }` |
