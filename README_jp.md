...
更新履歴: [docs/updates/README.md](docs/updates/README.md) / [beta 1.0.7 日本語](docs/updates/beta-1.0.7.ja.md)
...
| 領域 | 内容 |
|------|------|
| Hub | active / archived session 一覧、New Session、Revive、Settings |
...
| Backend | Auto mode、Awake、通知音と browser notification、Hub / chat の PWA 導線、secure browser feature 向けの local HTTPS |
...
Hub は active / archived session 一覧を管理する入口です。active session では workspace path、agent 構成、chat 数、chat port を見ながらそのまま chat UI を開けます。archived session も同じトップページ上に残り、再開可能なものは revive できます。Hub からは New Session と Settings へ移動できます。
...
Cron の実行経路は専用 transport ではなく、通常どおり pane に注入して実行します。定時実行の結果は通常作業と同じ native event log 取り込み経路で chat タイムラインに戻ります。scheduler は pending run を追跡し、一定時間 reply が来なければ 1 回 reminder を送り、それでも返答が無ければ timeout として記録します。
...
- [docs/updates/beta-1.0.7.ja.md](docs/updates/beta-1.0.7.ja.md): `beta 1.0.6` 以降の変更点
