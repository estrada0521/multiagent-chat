...
会話ログは `.agent-index.jsonl`、pane 側の表示は `.log` と `.ans` に残ります。Hub から session の作成・再開・統計確認・設定変更を行い、chat UI からターゲット選択、ファイル参照、brief、memory、pane 操作、export をまとめて扱えます。PC だけでなく、同一 LAN 上のスマホからも同じ Hub / chat UI を開けます。
...
| Chat UI | user と agent、agent 同士の会話、添付、ファイル参照、brief / memory、pane 操作、runtime ヒント表示、スマホ向けの camera mode |
